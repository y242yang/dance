...
--
-- Semantics:
--   * Rows in [p_today, p_covered_through] missing from this scrape are hard
--     deleted, unconditionally - including ones saved or committed-to by a user.
--     (2026-07-27: this used to soft-cancel a row still referenced by
--     saved_classes/log_entries instead of deleting it, specifically so a scraper
--     miss couldn't silently destroy a user's save or commitment. Removed by
--     explicit choice - see sql/social_schema.sql's 2026-07-27 migration for the
--     trade-off being accepted.) saved_classes/log_entries' own ON DELETE CASCADE
--     handles cleaning up any reference to the deleted row.
--   * Upsert the provided rows by id (stable ids keep saved/hearted classes valid
--     across runs where the row itself survives).
...
begin
    ...
    -- Hard delete: missing from this scrape. (v_ids IS NULL -> empty scrape ->
    -- delete the whole covered range for this studio.)
